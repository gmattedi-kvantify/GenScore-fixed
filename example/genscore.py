...
from GenScore.model.model import GatedGCN, GenScore, GraphTransformer
...
def Input():
    p = argparse.ArgumentParser(
        "Run Genscore to predict the binding affinity between protein and ligand",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("-p", "--prot", required=True, help="Input protein file (.pdb)")
    p.add_argument("-l", "--lig", required=True, help="Input ligand file (.sdf/.mol2)")
    p.add_argument(
        "-m",
        "--model",
        default="../trained_models/GT_0.0_1.pth",
    ...
    p.add_argument(
        ...
        default="gt",
        choices=["gt", "gatedgcn"],
        help="the feature encoders for the representation of proteins and ligands",
    )
    p.add_argument("-o", "--outprefix", default="out", help="the prefix of output file")
    p.add_argument(
        "-gen_pocket",
        "--gen_pocket",
        action="store_true",
    ...
    p.add_argument(
        ...
        default=10.0,
        type=float,
        help="the cutoff the define the pocket and interactions within the pocket",
    )
    p.add_argument(
        "-rl",
        "--reflig",
    ...
    p.add_argument(
        "--batch_size", default=128, type=int, help="the batch size for the evaluation"
    )
    p.add_argument(
        "--num_workers",
        default=10,
    ...
    **kwargs
    ...
    if atom_contribution:
        ...
        resids = [
            "%s_%s%s" % (x[0], y, z)
            for x, y, z in zip(
                u.residues.chainIDs, u.residues.resnames, u.residues.resids
            )
        ]
...
def main():
    inargs = Input()
    args = {}
    args["batch_size"] = inargs.batch_size
    args["dist_threhold"] = 5.0
    args["device"] = "cuda" if th.cuda.is_available() else "cpu"
    args["num_workers"] = inargs.num_workers
    ...
    if inargs.atom_contribution:
        ids, scores, atids, at_contrs = scoring(
            prot=inargs.prot,
            lig=inargs.lig,
        ...
        ids, scores, resids, res_contrs = scoring(
            prot=inargs.prot,
            lig=inargs.lig,
            modpath=inargs.model,
            cut=inargs.cutoff,
